...
class PackTrackInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if "img" in results:
            ...
            if is_seq_of(imgs, np.ndarray):
                imgs = np.stack(imgs, axis=0)
                imgs = imgs.transpose(0, 3, 1, 2)
                packed_results["inputs"] = to_tensor(imgs)
            elif is_seq_of(imgs, torch.Tensor):
                imgs = torch.stack(imgs, dim=0)
                imgs = imgs.permute(0, 3, 1, 2)
                packed_results["inputs"] = to_tensor(imgs)
            else:
                raise ValueError(f"Unsupported image type: {type(imgs)}")
            ...
            if key == "gt_masks":
                ...
                if "gt_ignore_flags" in results:
                    for i, ann in enumerate(anns_list):
                        ...
                        ignore_instance_data_list[i][self.mapping_table[key]] = (
...
class PackReIDInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if isinstance(results["img"], (list, tuple)):
            ...
            assert all(
                [isinstance(v, type(results["img"])) for v in results.values()]
            ), "All items in the results must have the same type."
        else:
            img = [results["img"]]

        if is_seq_of(img, np.ndarray):
            img = np.stack(img, axis=3)  # (H, W, C, N)
            img = img.transpose(3, 2, 0, 1)  # (N, C, H, W)
            img = np.ascontiguousarray(img)
            packed_results["inputs"] = to_tensor(img)
        elif is_seq_of(img, torch.Tensor):
            img = torch.stack(img, dim=3)  # (H, W, C, N)
            img = img.permute(3, 2, 0, 1)  # (N, C, H, W)
            packed_results["inputs"] = img
